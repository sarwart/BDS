...
look_up_x=zeros(length(tracks.data),lim_x);
look_up_y=zeros(length(tracks.data),lim_y);
look_up_z=zeros(length(tracks.data),lim_z);

 for i=1:length(tracks.data)

 streamline=abs(floor(tracks.data{i}./vox))+1;
...
parfor total=1:x_cord
    ...
    if length(check)>1
        for k=1:length(check) %number of streamlines
            ...
            temp_index=1:N; 
            streamline=floor(tracks.data{check(k)}./vox);
